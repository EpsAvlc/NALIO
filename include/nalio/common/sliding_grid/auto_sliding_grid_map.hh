...
#include "nalio/common/sliding_grid/grid_map.hh"
...
class AutoSlidingGridMap : public GridMap<Dim, GridType> {
 public:
  ...
  AutoSlidingGridMap(const Config& config) : config_(config), BaseT(config) {
    NALIO_EXPECT_TRUE(config_.resolution > 0);
    NLOG_INFO("Grid size: %ld", grids_.size());
  ...
  IndexXd locToIndexXd(const VectorX& x) const override {
    ...
    for (size_t di = 0; di < Dim; ++di) {
      NALIO_EXPECT_TRUE(x(di) >= origin_(di) && x(di) <= origin_(di) + config_.size[di]);
      grid_ind_per_dim[di] =
 ...
 private:
  ...
  void clearGrids(const size_t dim, size_t start, size_t end) {
    NALIO_EXPECT_TRUE(start >= 0 && start <= grid_size_[dim] && end >= 0 && end <= grid_size_[dim]);
    // size_t range_end = end <= start ? end + grid_size_[dim] : end;
    std::vector<size_t> v_possible_range_start{start, start, end};
    std::vector<size_t> v_possible_range_end{end, end + grid_size_[dim], start};

    size_t range_start = 0, range_end = 0;
    size_t min_range = grid_size_[dim] + 1;
    for (size_t pi = 0; pi < v_possible_range_start.size(); ++pi) {
      int clear_range = static_cast<int>(v_possible_range_end[pi] - v_possible_range_start[pi]);
    ...
    NLOG_INFO("Clear grids in dim %ld from %ld to %ld, min_range: %ld.", dim, range_start, range_end, min_range);
